...
- **Zero Dependencies:** Runs entirely on standard `bash`, `curl`, and standard library `python` (for JSON parsing, compatible with Python 2 and 3). No virtual environments or packages needed.
...
- Bash
- `curl`
- A basic Python installation (pre-installed on almost all Unix-like platforms) for parsing response payloads.
